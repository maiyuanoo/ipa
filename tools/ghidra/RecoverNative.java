...
public class RecoverNative extends GhidraScript {
    ...
    private Set<Long> getEvidenceReferenceOffsets(Set<String> evidenceStrings) {
    ...
    private Set<String> getJellyDrawingEvidenceStrings() {
        Set<String> evidenceStrings = new HashSet<String>();
        String[] values = {
            "entity_draw", "Chest", "Item", "Loot", "Unknown Item", "Enemy", "Monster",
            "%s %.0fm", "%s %u %.0fm", "物资"
        };
        for (String value : values) evidenceStrings.add(value);
        return evidenceStrings;
    ...
    public void run() throws Exception {
        ...
        if (arguments.length < 1 || arguments.length > 2) {
            throw new IllegalArgumentException("Expected output file path and optional drawing or jelly mode");
        }
        ...
        Set<Long> evidenceReferenceOffsets = getEvidenceReferenceOffsets(evidenceStrings);
        ...
        try (PrintWriter writer = new PrintWriter(outputFile, "UTF-8")) {
            ...
            writer.printf("/* Evidence strings: %s */%n", evidenceStrings);
